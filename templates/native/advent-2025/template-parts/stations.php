...
<?php elseif ($style == 'blue'): ?> bg-[#122339] <?php endif; ?>">
    ...
    <div class="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
        ...
        <?php elseif ($style == 'dark-green'): ?>
            <img class="absolute right-0 top-0 max-w-[280px]" src="<?= $native_path; ?>/assets/stations/green-motiv.webp" alt="Green Motiv">
    ...
    <div class="max-w-[640px] w-full mx-auto flex flex-col gap-[42px] relative z-10 px-4">
        <div class="flex flex-col gap-[36px]">
            <h2 class="font-christmas text-[39px] md:text-[60px] leading-[64px] text-center text-light-sand md:underline"><?= $title; ?></h2>
            <p class="text-[16px] md:text-[20px] font-poppins text-center text-light-sand md:underline">
                <?= $description; ?>
            </p>
        </div>
        <button id="toggleBtn-<?= $block_id; ?>" class="bg-white px-5 py-[9px] rounded-[3px] shadow-sm w-fit mx-auto items-center text-black uppercase text-[16px] md:text-[20px] font-bold leading-6 flex flex-row gap-1 font-lato z-40">
            <span class="md:underline">Posjeti ih</span>
    ...
    <?php if ($style == 'dark-green'): ?>
        <div class="absolute block bottom-[-25%] right-[25%] z-[4]">
            <svg id="path-darkgreen" class="hidden lg:block" width="177" height="462" viewBox="0 0 177 462" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M23 1L25 20C26 32 26 44 30 55C34 67 43 77 53 86L164.1 224.423C187.232 244.963 167.906 282.829 137.701 276.148L131.95 274.876C105.287 268.977 85.2501 298.873 100.84 321.294L137.63 374.205C148.265 389.499 142.536 410.687 125.643 418.537L37.5 459.5"
                    stroke="white" stroke-opacity="0.25" stroke-width="4" stroke-dasharray="12 12" />
            </svg>
    ...
    <?php elseif ($style == 'green'): ?>
        <div class="absolute block bottom-[-25%] right-[25%] z-[4]">
            ...
            <svg id="path-green-mobile" class="lg:hidden" width="136" height="343" viewBox="0 0 136 343" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <?php elseif ($style == 'bordo'): ?>
        <div class="absolute block bottom-[-20%] left-[25%] z-[4]">
    ...
    <!-- Polaroids -->
    <div id="polaroids-<?= $block_id ?>" class="hidden flex-col md:flex-row gap-12 relative z-20 [&>div]:first:rotate-[-3deg] [&>div]:last:rotate-[3deg]" style="opacity: 0; max-height: 0; overflow: hidden; transition: opacity 0.6s ease-in-out, max-height 0.6s ease-in-out, transform 0.6s ease-in-out; transform: translateY(-20px);">
