<?php $block = wp_cache_get('telesport_block', 'blocks');
if (!$block) {
	ob_start();
	?>
    <div class="block block-telesport">
        <div class="container">
            ...
            <div class="block-body feed-container">
				<?php
				switch_to_blog( 2 );
				$articles = z_get_zone_query('telegram-homepage', ['post_type' => array( 'post', 'kolumne', 'na-prvu' )]);
                $out = [];
				...
				$args     = array(
					'posts_per_page' => 3,
					'no_found_rows'  => true,
					'post_type'      => array( 'post', 'kolumne', 'na-prvu' ),
					'post_status'    => 'publish',
					'post__not_in' => $out
				);
...
?>
	...
	$block = ob_get_clean();
	wp_cache_set( 'telesport_block', 'blocks', 15 * 60 );
}
