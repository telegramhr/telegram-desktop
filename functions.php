...
function telegram_enqueue_styles() {
    wp_enqueue_style('telegram-style', get_stylesheet_uri(), array('slick', 'slick-theme'), '95050.37');
...
function telegram_scripts() {
    ...
    wp_enqueue_script('telegram-script', get_template_directory_uri() . '/assets/js/functions.js', array('jquery', 'slick', 'colour-brightness', 'sticky-kit'), '94916.38', true);
...
function telegram_price_content($content){
    if (is_amp_endpoint() || get_query_var('audit')) {
        return $content;
    }
    if (is_single()) {
