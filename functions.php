...
function telegram_scripts() {
    ...
    wp_register_script('telegram-script', get_template_directory_uri() . '/assets/js/functions.js', array('jquery', 'slick', 'colour-brightness', 'sticky-kit'), '95137.98', true);
	wp_register_style('telegram-style', get_stylesheet_uri(), array('slick', 'slick-theme'), '95070.22');
...
function telegram_price_content($content){
    ...
    global $post;
    if (!in_array($post->post_type, [ 'post', 'price' ])) {
        return $content;
    }
    $new_line = "</p>";
