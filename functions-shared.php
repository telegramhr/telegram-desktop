    ...
    if ( $id ) {
        ...
        $content = preg_replace_callback('/<a([^>]*)href=["|\']([^"|\']*)["|\'][^>]*>([^<]*)<\/a>/i', function ($m) use ($post, $cat, $id) {
            if (strpos($m[2], "www.telegram.hr") === false) {
                ...
                if (in_array($id, [2616854, 2616789, 2586227, 2440142, 2393151, 2373566, 2384343,2388443, 1733848, 1733874, 1732851, 1768545, 1808006, 1808023, 1808011, 1837766, 1839950, 1850741, 1866509, 1891441, 1898612, 1929302,1957325, 1982562, 1990700, 2014673,2021770, 2049906, 2151405, 2220119, 2245739, 2273990, 2307537, 1624246, 2355120, 2373566])) {
                    $rel = '';
                }
